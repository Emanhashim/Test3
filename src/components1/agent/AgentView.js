...
import MerchantMainView from "../merchant/MerchantMainView"
const AgentView = () => {
  useEffect(() => {
  ...
  const loadUser = async () => {
    setLoading(true);
    const res = await axios.get(`http://198.199.67.201:8080/Api/Accounts/Agent/${id}`
    ,{ headers: {
...
if (loading) {
  ...
  return (
    ...
    <div className="title" >
 <h4 > <HiOutlineIdentification style={{ fontSize: "20px"  }}/> Agent ID: {id}</h4>
 </div>
 <hr />
